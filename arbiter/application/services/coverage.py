...
def check_coverage(
    ...
    *,
    ...
    if not crit:
        ...
        if record:
            _record_coverage(app, result, tool=tool, paths=paths, actor=actor, at=moment)
        return result
    ...
    if break_glass:
        ...
        if record:
            app.commands.record_break_glass(
                BreakGlassUsed(
    ...
    if decision_id:
        covered, uncovered, reason = _evaluate_decision(app, decision_id, crit, moment)
        result = {
            "approved": covered,
        ...
        if record:
            _record_coverage(app, result, tool=tool, paths=crit, actor=actor, at=moment)
        return result

    for did in _decision_ids(app):
        covered, uncovered, reason = _evaluate_decision(app, did, crit, moment)
        if covered:
            result = {
                "approved": True,
                "path": "covered",
...
def _evaluate_decision(
    app: Any, decision_id: str, paths: list[str], moment: datetime
) -> tuple[bool, list[str], str]:
    state = app.replay(decision_id)
    if state is None:
...
def _decision_ids(app: Any) -> list[str]:
    seen: list[str] = []
...
def _record_coverage(
    ...
    *,
    ...
    app.commands.record_coverage_checked(
        CoverageChecked(
            at=format_iso(at),
            tool=tool,
            paths=tuple(paths),
