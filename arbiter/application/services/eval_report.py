...
class EvalReportBuilder:
    ...
    def build(self) -> dict[str, Any]:
        ...
        divergence = self._divergence(opens, resolved, baselines)
        internal = self._internal_dissent(opens, round2, votes)
        reveal = self._reveal_efficacy(opens, round2, votes, resolved)
        cover = self._coverage_mix(holds, coverage)
        cost = self._cost_time(votes, holds)
        glass_dist = self._glass(glass)
        reversals = self._reversibility(resolved, baselines)

        thesis = self._thesis(divergence, reversals)
        ...
        return {
            ...
            "thesis": thesis,
            "formulation_notes": self._formulation_notes(opens, resolved, round2, holds),
        }
    ...
    def _coverage_mix(
        self, holds: list[dict], coverage: list[dict]
    ) -> dict[str, Any]:
    ...
    def _cost_time(self, votes: list[dict], holds: list[dict]) -> dict[str, Any]:
    ...
    def _glass(self, glass: list[dict]) -> dict[str, Any]:
    ...
    def _reversibility(
        ...
        commits = self._git_decision_commits()
        ...
        for did, res in resolved.items():
            ...
            if res.get("verdict") == "deny" and b_opt == "allow":
                q_deny_b_allow_n += 1
                if self._reversed_after(did, commits, horizon):
        ...
        return {
            ...
            "method": (
                f"git log trailers matched to later commits within {self.horizon_days}d "
                "whose subject/body match revert|hotfix|rollback"
            ),
        }

    def _git_decision_commits(self) -> list[dict[str, Any]]:
        repo = self.repo or Path.cwd()
    ...
    def _thesis(self, divergence: dict, reversals: dict) -> dict[str, Any]:
        ...
        return {
            ...
            "caveats": [
                ...
                f"Reversibility horizon is {self.horizon_days} days — hotfixes later are invisible.",
